...
export interface RequestorCore {
...
export interface RequestorParams {
  requestor?: Requestor;
  urlBase?: string;
  params?: Object;
  headers?: Object;
...
export class BaseRequestor extends Requestor {
  ...
  async sendJSON<T extends any>(url: string, params?: Object, postData?: Object): Promise<T> {
    return this.sendData(url, params, JSON.stringify(postData))
      .then(s => {
        return JSON.parse(this.decrypt(s));
      });
  }
}
...
class RequestorImpl extends Requestor {
  async sendData(url: string, params?: Object, postData?: string, opts?: Options): Promise<string> {
    return new Promise<string>((resolve, reject) => {
      ...
      req.post(postData, (err, data: string) => {
        if (err) {
          reject(err);
        } else {
          resolve(data);
